    ...
    <div class="bg-white dark:bg-[#121214] border border-slate-200 dark:border-zinc-800/90 rounded-3xl p-6 sm:p-7 shadow-xl dark:shadow-2xl relative overflow-hidden backdrop-blur-xl transition-colors">
        ...
        <div x-show="currentStep === 5" x-transition.opacity.duration.300ms class="space-y-6 text-center py-2">
            ...
            <div class="space-y-3 text-left">
                ...
                <!-- Indicator Mastery Table -->
                <div class="bg-slate-50 dark:bg-[#18181c] border border-slate-200 dark:border-zinc-800/90 rounded-xl p-4 flex flex-col gap-2">
                    <span class="text-sm font-medium text-slate-600 dark:text-zinc-400">Penguasaan Indikator</span>
                    <div class="space-y-1.5 mt-2">
                        <template x-for="mastery in (examResult?.indicator_mastery || [])" :key="mastery.indikator">
                            <div class="flex items-center justify-between text-xs border-b border-slate-200 dark:border-zinc-800 pb-1.5 last:border-0 last:pb-0">
                                <span class="font-semibold text-slate-700 dark:text-zinc-300" x-text="mastery.indikator"></span>
                                <div class="flex items-center gap-2">
                                    <span class="font-bold text-slate-900 dark:text-white" x-text="`${mastery.correct}/${mastery.total} (${mastery.percent}%)`"></span>
                                    <span :class="mastery.mastered ? 'text-emerald-500' : 'text-rose-500'" x-text="mastery.mastered ? '✅' : '❌'"></span>
                                </div>
                            </div>
                        </template>
                        <template x-if="!(examResult?.indicator_mastery?.length)">
                            <span class="text-xs text-slate-500 italic">Tidak ada data indikator.</span>
                        </template>
                    </div>
                </div>
            </div>
            ...
            <div class="flex-1 overflow-y-auto py-4 space-y-6 pr-1">
                <template x-for="item in examResult?.review" :key="item.number">
                    <div class="bg-slate-50 dark:bg-[#1a1a1e] border rounded-2xl p-5 space-y-3"
                        ...
                        <!-- Diagnostic Info -->
                        <template x-if="!item.is_correct && (item.error_pattern || item.diagnosis || item.treatment)">
                            <div class="bg-rose-50 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-900/40 rounded-xl p-3 space-y-2 mt-2">
                                <span class="text-xs font-bold text-rose-700 dark:text-rose-400 block flex items-center gap-1.5">
                                    <span>🩺</span> Analisis Kesalahan:
                                </span>
                                <div class="grid grid-cols-1 gap-2 text-xs">
                                    <template x-if="item.error_pattern">
                                        <div><span class="font-semibold text-slate-600 dark:text-zinc-400">Pola Kesalahan:</span> <span class="text-slate-800 dark:text-zinc-200" x-text="item.error_pattern"></span></div>
                                    </template>
                                    <template x-if="item.diagnosis">
                                        <div><span class="font-semibold text-slate-600 dark:text-zinc-400">Diagnosis:</span> <span class="text-slate-800 dark:text-zinc-200" x-text="item.diagnosis"></span></div>
                                    </template>
                                    <template x-if="item.treatment">
                                        <div><span class="font-semibold text-slate-600 dark:text-zinc-400">Rekomendasi Belajar:</span> <span class="text-slate-800 dark:text-zinc-200" x-text="item.treatment"></span></div>
                                    </template>
                                </div>
                            </div>
                        </template>
                        ...
                        <div class="pt-3 border-t border-slate-200 dark:border-zinc-800/80 bg-slate-100 dark:bg-zinc-900/60 -mx-5 -mb-5 p-4 rounded-b-2xl">
                            ...
                            <p class="text-xs text-slate-700 dark:text-zinc-300 leading-relaxed whitespace-pre-line" x-text="item.explanation"></p>
                        </div>
                    </div>
                </template>
            </div>
